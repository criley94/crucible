...
description: >
  Use when the sponsor triggers an experience layer review, after sprint retrospectives, after feature completion, or when institutional knowledge is at risk of being lost. Captures decisions, tradeoffs, lessons learned. Curates experience entries in the TeamForge database. Do NOT spin up on autopilot — only when the sponsor initiates it.
model: inherit
...
```
curl -s -H "X-API-Key: $API_KEY" -H "Authorization: Bearer $TOKEN" "$API_URL/api/v1/agents/quinn-historian"
```
